<?php
 ...
 if(isset($_POST["sign_up"])){
    ...
    function authenticateUser($email,$pass){
		$query = "select * from add_emp where email='$email' and pass='$pass'";
		$rs = get($query);
		if(count($rs)>0){
			return true;
		}
    ...
    function insertUser($name,$email,$pas,$phn,$nid,$adrs,$year,$month,$bdate,$blood_group,$gender,$opsn,$health){
		var_dump($health);
        $query = "insert into emp_info values(NULL,'$name','$email','$pas','$phn','$nid','$adrs','$year.$month.$bdate','$blood_group','$gender','$opsn','$health')";
